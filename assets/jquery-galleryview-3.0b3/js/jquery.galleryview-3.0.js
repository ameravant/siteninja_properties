...
(function(jQuery){
	jQuery.fn.galleryView = function(options) {
		var opts = jQuery.extend(jQuery.fn.galleryView.defaults,options);
...
*/
	...
	*/
		function showItem(i,speed,callback) {
			...
			jQuery('.gv-nav-next, .gv-panel-nav-next, .gv-nav-prev, .gv-panel-nav-prev',j_gallery).unbind('click');
			j_frames.unbind('click');
			
			// Use timer to rebind navigation buttons when transition ends
			jQuery(document).oneTime(speed,'bindNavButtons',function(){
				jQuery('.gv-nav-next, .gv-panel-nav-next',j_gallery).click(showNextItem);
				jQuery('.gv-nav-prev, .gv-panel-nav-prev',j_gallery).click(showPrevItem);
				enableFrameClicking();
			});
				...
				if(animate_panels) {
					if(opts.panel_animation=='slide') {
						...
						j_panels.eq(mod_i).css({
							left: getInt(jQuery('.gv-panel.current').eq(0).css('left'))+opts.panel_width+'px',
							zIndex: 50
						}).show().animate({
						...
						jQuery('.gv-panel.current').css({zIndex:49}).animate({
							left: '-='+opts.panel_width+'px'
						},speed,opts.easing,function(){
							jQuery(this).removeClass('current').hide();
						});
						...
						jQuery(document).oneTime(speed,'setCurrentFrame',function(){
						...
						jQuery('.gv-panel.current img').animate({
						...
						jQuery('.gv-panel.current').animate({
							top:'+='+opts.panel_height/2+'px',
							left:'+='+opts.panel_width/2+'px',
							height:0,
							width:0
						},speed,'swing',function(){
							jQuery(this).removeClass('current').hide().css({
						...
						j_panels.removeClass('current').fadeOut(speed,function(){jQuery(this).css('filter','');}).eq(mod_i).addClass('current').fadeIn(speed,function(){jQuery(this).css('filter','');});
					} else {
						...
						jQuery(document).oneTime(speed/2,'fadeInPanel',function(){
					...
					jQuery(document).oneTime(speed,'switch_panels',function(){j_panels.hide().eq(mod_i).show();});
				}
			}
			...
			if(opts.show_filmstrip) {
				...
				if(opts.filmstrip_style == 'scroll' && slide_method=='strip') {
					...
					if(filmstrip_orientation=='horizontal') {
						...
						//Determine distance between pointer (eventual destination) and target frame
						distance = getPos(j_frames[i]).top-getPos(jQuery('.gv-strip_wrapper',j_gallery)[0]).top;
						diststr = (distance>=0?'-=':'+=')+Math.abs(distance)+'px';
			...
			if(callback) {jQuery(document).oneTime(speed,'showItemCallback',callback);}
			current = i;
		};
	...
	*/
		function showNextItem() {
			...
			if(!paused && opts.transition_interval > 0) {
				jQuery(document).everyTime(opts.transition_interval,"transition",function(){showNextItem();});
			}
		};
	...
	*/
		function buildPanels() {
			...
			j_panels.each(function(i){
		   		if(jQuery('.gv-panel-overlay',this).length>0) {
					jQuery(this).append('<div class="gv-overlay-background"></div>');	
				}
		   	});
			
			// If there is no filmstrip in this gallery, add navigation buttons to the panel itself
			if(opts.show_panel_nav) {
				jQuery('<div></div>').addClass('gv-panel-nav-next').appendTo(j_gallery).css({
					position:'absolute',
					zIndex:'1100',
				...
				jQuery('<div></div>').addClass('gv-panel-nav-prev').appendTo(j_gallery).css({
					position:'absolute',
					zIndex:'1100',
					top:((opts.filmstrip_position=='top'?opts.frame_gap+wrapper_height:0)+(opts.panel_height-22)/2)+'px',
			...
			jQuery('.gv-panel-overlay',j_panels).css({
				position:'absolute',
				zIndex:'999',
				width:(opts.panel_width-extraWidth(jQuery('.gv-panel-overlay',j_panels)))+'px',
				left:0
			});
			jQuery('.gv-overlay-background',j_panels).css({
				position:'absolute',
				zIndex:'998',
				width:opts.panel_width+'px',
				left:0,
			...
			if(opts.overlay_position=='top') {
				jQuery('.gv-panel-overlay',j_panels).css('top',0);
				jQuery('.gv-overlay-background',j_panels).css('top',0);
			} else {
				jQuery('.gv-panel-overlay',j_panels).css('bottom',0);
				jQuery('.gv-overlay-background',j_panels).css('bottom',0);
			}
			
			jQuery('.gv-panel iframe',j_panels).css({
				width:opts.panel_width+'px',
				height:opts.panel_height+'px',
				border:0
			});
			...
			if(scale_panel_images) {
				jQuery('img',j_panels).each(function(i){
					jQuery(this).css({
	...
	*/
		function buildFilmstrip() {
			...
			if(opts.filmstrip_style == 'scroll' && slide_method=='strip') {
				...
				j_frames.clone().appendTo(j_filmstrip);
				j_frames = jQuery('li',j_filmstrip);
			}
			...
			if(opts.show_captions) {
				j_frames.append('<div class="gv-caption"></div>').each(function(i){
					jQuery(this).find('.gv-caption').html(jQuery(this).find('img').attr('title'));	
				});
			}
			...
			jQuery('.gv-img_wrap',j_frames).each(function(i){								  
				jQuery(this).css({
			...
			jQuery('img',j_frames).each(function(i){
				jQuery(this).css({
					...
					if(!jQuery(this).parent().parent().hasClass('current')){jQuery(this).stop().animate({opacity:opts.frame_opacity},300);}
				});
			});
			
			// Set overflow of filmstrip wrapper to hidden so as to hide frames that don't fit within the gallery
			jQuery('.gv-strip_wrapper',j_gallery).css({
				position:'absolute',
				overflow:'hidden'
			});
			...
			jQuery('.gv-caption',j_gallery).css({
				position:'absolute',
			...
			var transColor = jQuery.browser.msie && jQuery.browser.version.substr(0,1) == '6' ? 'pink' : 'transparent';
			...
			j_pointer = jQuery('.gv-pointer',j_gallery);
			
			// Add navigation buttons
			if(opts.show_filmstrip_nav) {
				var navNext = jQuery('<div></div>');
				navNext.addClass('gv-nav-next').appendTo(j_gallery).css({
					position:'absolute'
				}).click(showNextItem);
				
				var navPrev = jQuery('<div></div>');
	...
	*/
		function mouseIsOverPanel(x,y) {
			...
			var pos = getPos(jQuery('#'+id+' .gv-panel_wrap')[0]);
			var gPos = getPos(j_gallery[0]);
	...
	*/
		function buildGallery() {
			...
			// Get gallery images
			var gallery_images = opts.show_filmstrip?jQuery('img',j_frames):jQuery('img',j_panels);
			...
			if(!opts.show_overlays) {
				jQuery('.gv-panel-overlay',j_gallery).remove();	
			}
				...
				jQuery(document).mousemove(function(e){
					...
					if(opts.pause_on_hover) {
						if(mouseIsOverGallery(e.pageX,e.pageY) && !paused) {
							...
							jQuery(document).oneTime(500,"animation_pause",function(){
								jQuery(document).stopTime("transition");
								paused=true;
							});
						} else {
							...
							if(paused && opts.transition_interval > 0) {
								jQuery(document).everyTime(opts.transition_interval,"transition",function(){
									showNextItem();
								});
								paused = false;
							}
					...
					if(opts.show_panel_nav) {
						if(mouseIsOverPanel(e.pageX,e.pageY) && !panel_nav_displayed) {
							jQuery('.gv-panel-nav-next, .gv-panel-nav-prev',j_gallery).show();
							panel_nav_displayed = true;	
						} else if(!mouseIsOverPanel(e.pageX,e.pageY) && panel_nav_displayed) {
							jQuery('.gv-panel-nav-next, .gv-panel-nav-prev',j_gallery).hide();
			...
			showItem(iterator,10,function(){
				jQuery('.gv-loader',j_gallery).fadeOut('1000',function(){
	...
	*/
		return this.each(function() {
			...
			scale_panel_images = jQuery('.gv-panel-content',j_gallery).length==0;
			...
			j_filmstrip = jQuery('.gv-filmstrip',j_gallery);
			j_frames = jQuery('li',j_filmstrip);
			j_frames.addClass('gv-frame');
			j_panel_wrapper = jQuery('<div>');
			...
			if(opts.show_panels) {
				...
				jQuery('.gv-panel-overlay',j_frames).remove(); 
				jQuery('.gv-panel-content',j_frames).remove();
			}
			...
			if(!opts.show_filmstrip) { j_filmstrip.remove(); }
				...
				j_frames.each(function(i){
					if(jQuery(this).find('a').length>0) {
						jQuery(this).find('a').wrap('<div class="gv-img_wrap"></div>');
					} else {
						jQuery(this).find('img').wrap('<div class="gv-img_wrap"></div>');	
					}
				});
				j_frame_img_wrappers = jQuery('.gv-img_wrap',j_frames);
			}
			
			j_panels = jQuery('.gv-panel',j_gallery);
			...
			jQuery('<div class="gv-caption"></div>').appendTo(j_frames);
			
			// Determine final frame dimensions, accounting for user-added padding and border
			f_frame_width = opts.frame_width+extraWidth(j_frame_img_wrappers);
			f_frame_height = opts.frame_height+extraHeight(j_frame_img_wrappers);
			frame_caption_size = getInt(jQuery('.gv-caption',j_gallery).css('height'));
			f_caption_width = f_frame_width - extraWidth(jQuery('.gv-caption',j_gallery));
			f_caption_height = frame_caption_size + extraHeight(jQuery('.gv-caption',j_gallery));
			
			// Delete the temporary caption element
			jQuery('.gv-caption',j_gallery).remove();
			...
			jQuery('<div>').addClass('gv-loader').css({
				position:'absolute',
				zIndex:'32666',
				opacity:1,
				top:0,
